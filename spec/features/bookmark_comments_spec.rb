feature 'comments' do
  ...
  scenario 'create a comment' do
    visit('/bookmarks')
    within find(:xpath, '//div[@class="bookmark"][2]') do
  ...
  scenario 'delete a comment' do
    visit('/bookmarks')
    expect(find(:xpath, '//div[@class="bookmark"][3]')).to have_content('Yaaaaas')
    find(:xpath, '//div[@class="bookmark"][3]').click_on('Delete Comment')
    expect(find(:xpath, '//div[@class="bookmark"][3]')).not_to have_content('Yaaaaas')
  end
end
